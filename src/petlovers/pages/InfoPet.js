...
// Função para obter o whatsapp do tutor pelo ID do pet
async function getTutorWhatsappByPetId(petId, userId) {
  try {
    const response = await fetch(`${baseAPI_URL}/api/match/${petId}/${userId}`);
    const data = await response.json();
    return data.whatsapp;
  } catch (error) {
    console.error("Erro ao obter o whatsapp do tutor associado ao pet:", error);
    return null;
  }
}
...
export default function InfoPet({ route }) { //rota com os parametros para renderizar o pet clicado 
  ...
  const [tutorWhatsapp, setTutorWhatsapp] = useState(null);
  ...
  const getTutorWhatsapp = async () => {
    try {
      var whatsapp = await getTutorWhatsappByPetId(id, userId);
      if (whatsapp) {
